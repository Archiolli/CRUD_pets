...
module.exports = class PerController {
  ...
  static async removePetById(req, res) {
    ...
    //check se o usuario ta logado registrou o pet
    const token = getToken(req)
    const user = await getUserByToken(token)

    if (pet.user._id.toString !== user._id.toString) {
      res.status(422).json({ message: 'Houve um problema em processar a solicitação, tente novamente mais tarde' })
      return
    }
  ...
  static async schedule(req, res) {
    ...
    //se o usuario ja agendou uma visita
    if (pet.adopter) {
    ...
    res.status(200).json({
      message: `A visita foi agendada com sucesso, entre em contato ${pet.user.name} pelo telefone ${pet.user.phone}`
    })
  }
  ...
  static async concludeAdoption(req, res) {
    ...
    //pet setado pra nao disponiivel
    pet.available = false
